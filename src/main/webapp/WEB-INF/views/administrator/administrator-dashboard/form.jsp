...
<br><br><br><br>
<h2>
	<acme:message code="administrator.dashboard.form.title.pimpam-data"/>
</h2>
...
<table class="table table-sm">
	...
	<tr>
		<th scope="row">
			<acme:message code="administrator.dashboard.form.label.ratioOfIngredientsWithPimpam"/>
		</th>
		<td>
			<acme:print value="${ratioOfIngredientsWithPimpum}"/>
		</td>
	</tr>
	
	<tr>
		<th scope="row">
			<acme:message code="administrator.dashboard.form.label.desviationPimpam"/>
		</th>

		<td>
			<jstl:forEach items="${pimpams}" var="pimpam">
				<jstl:choose>
					<jstl:when test="${pimpam.value.maximum.amount==0}">
						<em><acme:message code="administrator.dashboard.form.no-data"/> ${ingredient.key}</em>/
					</jstl:when>
					<jstl:otherwise>
						<acme:print value="${pimpam.value.desviation}"/>/
					</jstl:otherwise>
				</jstl:choose>
			</jstl:forEach>
		</td>
...
<br><br>
<h3>
	<acme:message code="administrator.dashboard.form.title.avg-data-pimpam"/>
</h3>
<br>
		...
		var data = {
			...
			datasets : [
				{	
					...
					data : [
						<jstl:forEach items="${pimpams}" var="pimpam">
							<jstl:out value="${pimpam.value.average.amount}"/>,
						</jstl:forEach>
					]
				}
			]
...
<%----------------------------PIMPAMS MAXS AND MINS --%>

<h3>
	<acme:message code="administrator.dashboard.form.title.min-max-dataPimpam"/>
</h3>
...
<script type="text/javascript">
	$(document).ready(function() {
		...
		var data = {
			...
			labels : [
				<jstl:forEach items="${pimpams}" var="pimpam">
					"MIN-"+"${pimpam.key}"+"-PIMPAM",
					"MAX-"+"${pimpam.key}"+"-PIMPAM",
				</jstl:forEach>
					],
			datasets : [
				{	
					...
					data : [
						<jstl:forEach items="${pimpams}" var="pimpam">
							<jstl:out value="${pimpam.value.minimum.amount}"/>,
							<jstl:out value="${pimpam.value.maximum.amount}"/>,
						</jstl:forEach>
					]
				}
			]
